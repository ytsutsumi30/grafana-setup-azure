...
async function upsertBalance(client, fields) {
    const found = await client.query(`
        SELECT id FROM inventory_balances
        WHERE product_id = $1
          AND COALESCE(lot_inventory_id, 0) = COALESCE($2::int, 0)
          AND COALESCE(qr_unit_id, 0) = COALESCE($3::int, 0)
          AND COALESCE(lot_number, '') = COALESCE($4, '')
          AND COALESCE(location_code, '') = COALESCE($5, '')
          AND inventory_status = $6
        LIMIT 1
    `, [fields.product_id, fields.lot_inventory_id || null, fields.qr_unit_id || null, fields.lot_number, fields.location_code, fields.inventory_status]);
    if (found.rows.length) {
        await client.query(`
            ...
            WHERE id = $3
        `, [fields.quantity_delta, fields.last_transaction_id, found.rows[0].id]);
        return;
    }
    await client.query(`
        INSERT INTO inventory_balances
          (product_id, lot_inventory_id, qr_unit_id, lot_number, location_id, location_code,
           inventory_status, quantity, last_transaction_id)
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
    `, [
        fields.product_id,
        fields.lot_inventory_id || null,
        fields.qr_unit_id || null,
        fields.lot_number,
...
router.post('/:id/scan', async (req, res) => {
    ...
    try {
        ...
        const lot = await client.query(`
            ...
            VALUES ($1,$2,$3,$4,$5,'available','available','入庫検品から作成')
            ON CONFLICT (product_id, lot_number) DO UPDATE SET
              ...
              updated_at = CURRENT_TIMESTAMP
            RETURNING *
        `, [line.product_id, value.lot_number, acceptedQuantity, value.location_code, loc.rows[0].id]);
        ...
        if (value.qr_code) {
            const qrResult = await client.query(`
                ...
                ON CONFLICT (qr_code) DO UPDATE SET
                  ...
                  updated_at = CURRENT_TIMESTAMP
                RETURNING *
            `, [value.qr_code, line.product_id, lot.rows[0].id, value.lot_number, acceptedQuantity, value.location_code, loc.rows[0].id]);
        ...
        const result = await client.query(`
            INSERT INTO receiving_results
              (receiving_order_id, receiving_order_line_id, purchase_order_line_id, product_id,
               ...
               inspection_status, reason_code, comment)
        ...
        const tx = await client.query(`
            ...
            VALUES ('receiving','posted',$1,$2,$3,$4,$5,$6,$7,$8,'receiving_order',$9,$10,$11,$12,'receiving-orders-api')
            RETURNING *
        `, [line.product_id, lot.rows[0].id, qrUnit ? qrUnit.id : null, value.lot_number,
            loc.rows[0].id, value.location_code, acceptedQuantity, lot.rows[0].quantity,
            req.params.id, line.id, value.reason_code || null, value.comment || '入庫検品で合格数量を在庫反映']);
